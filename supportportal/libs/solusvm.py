...
class SolusVM:
        ...
        def sQuery(self, **kwargs):
                ...
                req = urllib2.Request('https://'+self.url+':5656/api/admin/command.php?'+urllib.urlencode(kwargs))
        ...
        def virtualServerState(self, vserverid, nostatus=False, nographs=False):
                ...
                response = self.sQuery(**data)
                return response

                result = {}
                result["node"] = response["node"][0]
                result["bandwidth"] = response["bandwidth"][0]
                result["internalips"] = response["internalips"]               
                result["mainipaddress"] = response["mainipaddress"][0] 
                result["state"] = response["state"][0] 
                result["hdd"] = response["hdd"][0]                
                result["trafficgraph"] = response["trafficgraph"][0]
                result["ipaddresses"] = response["ipaddresses"][0]
                result["memory"] = response["memory"][0]
                result["type"] = response["type"][0]
                return result
