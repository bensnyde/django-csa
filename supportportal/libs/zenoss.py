...
class Zenoss(object):
    ...
    def get_device_graphs(self, device_uid, drange=129600):
        """Get a list of all devices.

        """
        return self.__router_request('DeviceRouter', 'getGraphDefs',
                                     data=[{'uid': device_uid, 'drange': drange}])   

    def get_device_interfaces(self, device_uid):
        return self.__router_request('DeviceRouter', 'getComponents',
                                     data=[{'uid': device_uid, 'keys': ["interfaceName"]}])            

    def get_device_components(self, device_uid):
        """Get a list of all devices.
    ...
    def get_interface_form(self, device_uid):
        """Get a list of all devices.

        """
        return self.__router_request('DeviceRouter', 'getForm',
                                     data=[{'uid': device_uid}])   
    ...
    def find_device(self, device_name):
        ...
        all_devices = self.get_devices()['result']
